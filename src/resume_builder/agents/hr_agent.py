...
class HRAgent(BaseAgent):
    ...
    SYSTEM_PROMPT: str = (
        ...
        "the report's has_placeholders field from this tool. For each item in "
        "placeholders_found, add a descriptive entry to issues (e.g. 'Placeholder "
        "text found: [Name]'). If has_placeholders is True, issues must contain "
        "at least one entry describing what was found.\n\n"
        "IMPORTANT — de-duplication: check_grammar and assess_professionalism "
        "both flag weak phrases. Match duplicates by the weak phrase text itself "
        "(e.g. 'responsible for'), not by the full issue string. When the same "
        "phrase appears in both tools' results, keep the issue string from "
        "check_grammar and discard the duplicate from assess_professionalism. "
        "Do not surface duplicate feedback.\n\n"
