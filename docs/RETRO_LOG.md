...
### [2026-03-03] P3-T04 — HR Review Agent (PR #TBD)

**QA**: HRAgent test suite is solid with 31 tests across 3 classes covering init, review behavior, and model validation. The three isinstance-style assertions were a pattern slip — they tested type conformance instead of fixture-driven values, which means they would pass even if the mock returned the wrong boolean. Tightened to `is False` / `is True` checks: these are semantically unambiguous and appropriate since bool is a singleton type. The distinction matters: `assert result.field is False` cannot be fooled by falsy non-bool values. Going forward, any test that mocks Claude's response and checks a boolean field should assert the exact value from the fixture, not just the type.

**UI/UX**: The placeholders_found→issues mapping gap was the most actionable finding: a candidate receiving has_placeholders=True with an empty issues list would have no way to know what placeholder was found or where to look. The fix adds explicit SYSTEM_PROMPT instruction to translate each placeholders_found item into a descriptive issues entry. The de-duplication clarification (exact phrase-text matching, check_grammar takes priority) ensures deterministic behavior instead of LLM-discretion string comparison. These are exactly the gaps that surface during SYSTEM_PROMPT review but are invisible during GREEN phase testing with controlled mocks — the mock returns whatever JSON is in the fixture regardless of how the prompt instructs the model.

**DevOps**: HRAgent introduces no new attack surface — same BaseAgent infrastructure as QAAgent, stdlib json only, no new dependencies. The unguarded `text` parameter on `review()` carries the same MAX_TEXT_SIZE advisory as QAAgent from P3-T02. Both agents should be guarded simultaneously when the FastAPI endpoint is built in P3-T06 to keep them consistent. No credentials, no PII exposure, gitleaks clean.

**Architecture**: The pre-task refactor commit (3c982e6) cleanly separates the module-split concern before the RED commit, making the intent legible in git history. HRReport and QAReport follow the same Pydantic v2 patterns with one intentional semantic distinction: is_formatting_consistent defaults to True (consistent is the expected state; the tool flags deviations) while is_accessible defaults to False (accessible is earned; the tool confirms it). Both are correct for their domains. All structural invariants are clean for P3-T05.
