...
Key project facts:
- Templates live in `src/resume_builder/templates/` (Jinja2, autoescape=True)
- Static assets live in `src/resume_builder/static/`
- API routes will live in `src/resume_builder/api/`
- The output is a resume — a document, not an app — but WCAG still applies to the web interface

## Scope Gate — Answer This First

Check the diff for:
- Any file in `src/resume_builder/templates/`
- Any file in `src/resume_builder/static/`
- Any file in `src/resume_builder/api/`
